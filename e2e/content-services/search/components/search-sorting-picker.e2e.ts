...
describe('Search Number Range Filter', () => {
    ...
    const pngAModel = new FileModel({
        'name': resources.Files.ADF_DOCUMENTS.PNG.file_name,
        'location': resources.Files.ADF_DOCUMENTS.PNG.file_location
    });

    const pngDModel = new FileModel({
    ...
    [C277270] Should see the icon for ASC and DESC sort when search results are displayed in the search results page`, () => {
    ...
    it('[C277284] Should be able to sort the search results by "Modifier" ASC', () => {
        ...
        contentServices.getNodesDisplayed(this.alfrescoJsApi).then(async (nodeList) => {
            let modifierList = [];
